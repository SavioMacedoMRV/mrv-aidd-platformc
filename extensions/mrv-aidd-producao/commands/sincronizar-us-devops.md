...
1. Persist the sync map back to `.specify/extensions/mrv-aidd-producao/us-sync-map.json`.
    - Store:
...
1. **Replicate spec and contracts to paired repositories (maestro mode)**:
    - Read `.specify/extensions/mrv-aidd-producao/maestro-config.json`. If it does not exist or `role` is not `"maestro"`, skip this step entirely.
    - For each entry in the `targets` array where `syncSpec` is `true`:
      a. Resolve the target repository path from `targets[].path`.
      b. Resolve the active feature directory name from `FEATURE_DIR` (captured in step 1).
      c. Create the directory `<target-path>/specs/<feature-directory-name>/` if it does not exist.
      d. Copy the active `spec.md` to `<target-path>/specs/<feature-directory-name>/spec.md`.
         - Prepend a reference header at the top of the copied file:

           ```
           <!-- Replicated from maestro: <maestro-repo-name> | <ISO-8601-timestamp> -->
           <!-- Source: <absolute-path-to-original-spec> -->
           <!-- Do not edit this file directly. Changes should be made in the maestro repository. -->
           ```

      e. If `syncContracts` is `true` and a `contracts/` directory exists inside the active feature directory, copy the entire `contracts/` folder to `<target-path>/specs/<feature-directory-name>/contracts/`.
      f. If the copy fails (target path inaccessible, permission error, etc.), report the failure for that target and continue with the next target.
    - In `--dry-run` mode, report which files would be replicated to which targets but do not copy anything.
    - Report the replication result as part of the overall summary: files replicated, targets reached, and any failures.
