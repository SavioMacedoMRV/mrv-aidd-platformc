...
- Quando a execucao estiver focada em uma US especifica, conduza a implementacao assumindo uma branch filha da funcionalidade no formato `feature/<feature-branch>/usN` e, quando necessario, oriente o uso de `/speckit.mrv-aidd-producao.configurar-us USn` para cria-la.
- Nao oriente abertura de PR de uma branch de historia de usuario diretamente para `main`; o destino correto dessas branches e a branch base da feature, como `001-feature-name`.
- O PR para `main` deve ser orientado apenas a partir da branch base da feature, depois que as USs planejadas para a funcionalidade estiverem concluidas.
- Todas as mensagens voltadas ao usuario por este comando DEVEM estar em portugues do Brasil, preservando apenas caminhos, comandos, identificadores tecnicos, nomes de arquivos e termos oficiais que precisem permanecer no idioma original.