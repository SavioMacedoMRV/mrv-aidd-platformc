---
description: Esclarecer a spec preservando historias de frontend e aprofundando apenas o ownership de backend.
handoffs:
...
- Preserve historias owned pelo frontend verbatim.
- Ao atualizar historias owned pelo backend, preserve a estrutura `Como um`, `Eu quero`, `Para que`, `Descricao`, `Valor para o Negocio`.
- Garanta que cada historia de backend continue com titulo iniciado por `[BACK] ` e `**Azure DevOps Tags**: [BACK]`.
- Se surgir trabalho de frontend, atualize `## Frontend Follow-up` em vez de cruzar ownership.
